...
pub(crate) fn is_terminal_submit_call(item: &ResponseItem) -> bool {
    ...
    for item in items {
        match item {
            ResponseItem::Message { role, content, .. } => match role.as_str() {
                ...
                "developer" => {
                    if content.iter().all(is_contextual_dev_content_item) {
                        continue;
                    }
                    flush_pending_assistant_content(
                        &mut messages,
...
mod tests {
    ...
    fn first_user_message_is_wrapped_with_mini_prompt() {
        let prompt = Prompt {
            ...
            tools: vec![codex_tools::create_mini_swe_agent_bash_tool()],
            ..Prompt::default()
        };
    ...
    async fn text_only_response_is_replaced_with_retry_error() {
        ...
        let mut stream = inject_no_tool_call_format_error(ResponseStream { rx_event });
        assert!(matches!(
            stream.next().await,
