...
impl ChatWidget {
    ...
    pub(crate) fn show_footer_context_notice(&mut self, message: String, duration: Duration) {
        self.bottom_pane
            .show_contextual_notice(Line::from(message), duration);
    }
    ...
    fn on_dynamic_tool_call_response(&mut self, ev: DynamicToolCallResponseEvent) {
        ...
        let active_dynamic_cell: Option<&mut history_cell::DynamicToolCallCell> =
            self.active_cell.as_mut().and_then(|cell| {
                cell.as_any_mut()
                    .downcast_mut::<history_cell::DynamicToolCallCell>()
            });
        match active_dynamic_cell {
    ...
    /// The Open Interpreter flow is provider-first: users pick a provider,
    /// then a provider-specific model, then the reasoning/thinking setting.
