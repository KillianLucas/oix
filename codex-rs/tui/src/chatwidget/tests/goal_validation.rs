...
async fn goal_slash_command_accepts_objective_at_limit() {
    ...
    let goal_event = events.iter().find_map(|event| match event {
        AppEvent::SetThreadGoalObjective {
            thread_id,
            objective,
            ..
        } => Some((*thread_id, objective.clone())),
        _ => None,
    });
    let (actual_thread_id, actual_objective) =
        goal_event.expect("expected SetThreadGoalObjective event");
